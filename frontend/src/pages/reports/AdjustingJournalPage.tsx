...
export default function AdjustingJournalPage() {
  ...
  const fetchData = useCallback(async () => {
    ...
    try{
      const {data}=await apiClient.get('/api/v1/reports/adjusting-journal');
    ...
    }catch(err:any){ if(err.response?.status===401) navigate({ to: '/' }); setErrorMessage(err.response?.data?.message||err.message); setEntries([]); } finally{ setLoading(false); }
  }, [navigate]);
  ...
  const deleteEntry = async (entry:JournalEntry) => {
    ...
    try{ await apiClient.delete(`/api/v1/reports/adjusting-journal/${entry.id}`); setEntries(prev=>prev.filter(e=>e.id!==entry.id)); }catch(err:any){ setErrorMessage(err.response?.data?.message||'Failed delete'); }
  };
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="flex flex-wrap items-center justify-between gap-3">
        ...
        <div className="flex gap-2"><Button asChild size="sm" className="gap-1.5"><Link to="/adjusting-journal-entry"><IconPlus size={14}/> Add Entry</Link></Button><Button variant={editMode? 'secondary':'outline'} size="sm" className="gap-1.5" onClick={()=>setEditMode(p=>!p)} disabled={!entries.length}><IconPencil size={14}/> Edit</Button></div>
      </div>
