  ...
  return (
    <div className="space-y-6 p-4 md:p-6">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
        <Card><CardHeader className="flex-row items-center justify-between space-y-0 pb-2"><CardDescription>Financial Health Index</CardDescription><IconActivity size={18} className="text-primary" /></CardHeader><CardContent className="flex items-center gap-4"><div className="w-16 h-16 rounded-full border-4 border-primary grid place-items-center font-bold text-lg">{healthScore}</div><div><p className={cn('text-sm font-semibold', healthScore >= 80? 'text-emerald-500' : healthScore >= 60? 'text-sky-500' : 'text-amber-500')}>{healthScore >= 80? 'Excellent' : healthScore >= 60? 'Stable' : 'Attention'}</p><p className="text-xs text-muted-foreground">Based on net profit margin</p></div></CardContent></Card>
        <Card><CardHeader className="flex-row items-center justify-between space-y-0 pb-2"><CardDescription>Total Cash & Bank (Kumulatif)</CardDescription><IconWallet size={18} className="text-amber-500" /></CardHeader><CardContent><div className="text-2xl font-bold font-mono">{formatNumber(data.totalAssets)}</div><div className="text-xs text-muted-foreground flex gap-4 mt-1"><span>Cash: <b className="text-foreground">{formatNumber(data.totalCashOnHand)}</b></span><span>Bank: <b className="text-foreground">{formatNumber(data.totalBankBalance)}</b></span></div></CardContent></Card>
      </div>

      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
        <Card><CardHeader className="flex-row items-center justify-between space-y-0 pb-1"><CardDescription>Revenue</CardDescription><div className="w-7 h-7 rounded-full bg-emerald-500/10 text-emerald-500 grid place-items-center"><IconTrendingUp size={16} /></div></CardHeader><CardContent><div className="text-xl font-bold font-mono">{formatNumber(data.totalRevenue)}</div><p className="text-xs text-muted-foreground">Period Revenue</p></CardContent></Card>
        <Card><CardHeader className="flex-row items-center justify-between space-y-0 pb-1"><CardDescription>Expenses</CardDescription><div className="w-7 h-7 rounded-full bg-red-500/10 text-red-500 grid place-items-center"><IconTrendingDown size={16} /></div></CardHeader><CardContent><div className="text-xl font-bold font-mono">{formatNumber(data.totalExpenses)}</div><p className="text-xs text-muted-foreground">Period Expenses</p></CardContent></Card>
        <Card className="bg-primary text-primary-foreground"><CardHeader className="flex-row items-center justify-between space-y-0 pb-1"><CardDescription className="text-primary-foreground/70">Net Income</CardDescription><IconShieldCheck size={18} /></CardHeader><CardContent><div className="text-xl font-bold font-mono">{formatNumber(data.netIncome)}</div><p className="text-xs text-primary-foreground/70">{data.netIncome >= 0? 'Profit' : 'Loss'} for Period</p></CardContent></Card>
        <Card><CardHeader className="flex-row items-center justify-between space-y-0 pb-1"><CardDescription>Liabilities</CardDescription><div className="w-7 h-7 rounded-full bg-amber-500/10 text-amber-500 grid place-items-center"><IconCreditCard size={16} /></div></CardHeader><CardContent><div className="text-xl font-bold font-mono">{formatNumber(data.totalLiabilities)}</div><p className="text-xs text-muted-foreground">Kumulatif Hutang</p></CardContent></Card>
      </div>
...
export default function DashboardPage() {
  return <Suspense fallback={<div className="p-6"><Skeleton className="h-" /></div>}><DashboardContent /></Suspense>;
}
