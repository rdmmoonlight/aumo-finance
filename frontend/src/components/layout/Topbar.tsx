...
export default function Topbar({ verse }: { verse: QuranVerse }) {
  ...
  return (
    <header className="sticky top-0 z-10 flex h-auto min-h-12 w-full shrink-0 items-center justify-between gap-4 border-b border-white/[0.07] bg-[#0E0E0E]/90 px-4 py-2 backdrop-blur-xl lg:px-6">
      {/* BREADCRUMB */}
      ...
      {/* QURAN - 3 BARIS MAX +... */}
      <div className="relative ml-auto hidden min-w-0 max-w-[44%] shrink-0 justify-end md:flex lg:max-w-">
        <div className="group relative w-full text-right">
          {/* 3 baris: 1 baris Arab + 2 baris English */}
          <div className="flex flex-col items-end gap-0.5">
            <p className="line-clamp-1 w-full text-right font-arabic text- font-medium leading-tight text-white">
              {verse.textAr}
            </p>
          ...
          {/* Tooltip full kalau mau baca lengkap pas hover */}
          <div className="pointer-events-none absolute right-0 top-[calc(100%+10px)] z-50 hidden w- opacity-0 transition-all group-hover:block group-hover:opacity-100">
            <div className="rounded-xl border border-white/10 bg-[#151519] p-3 shadow-[0_8px_32px_rgba(0,0,0,0.5)]">
              <p className="text-right text- leading-relaxed text-white">
                {verse.textAr}
              </p>
