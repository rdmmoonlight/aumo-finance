  ...
  return (
    <aside className="w-64 border-r border-white/[0.06] bg-[var(--color-matte)] flex flex-col h-screen sticky top-0 shrink-0 select-none z-20">
      <div className="h-16 px-5 border-b border-white/[0.06] flex items-center gap-3 shrink-0">
        <div className="w-8 h-8 rounded-lg bg-[var(--color-matte-hover)] text-white grid place-items-center font-bold text-sm">A</div>
        <div className="flex flex-col">
          <span className="font-semibold text-sm leading-none text-white tracking-tight">Aumo Finance</span>
          <span className="text-[10px] text-zinc-500 uppercase tracking-[0.16em] font-medium mt-1">Accounting Suite</span>
        </div>
      </div>
      <ScrollArea className="flex-1 min-h-0">
        <div className="px-3 py-5 space-y-7">
          <div>
            <h2 className="px-3 mb-3 text-[11px] font-semibold text-zinc-500 uppercase tracking-[0.14em]">Main Domain</h2>
            <div className="space-y-0.5">{mainNavItems.map((i) => <NavItemLink key={i.path} item={i} />)}</div>
          </div>
          <div className="h-px bg-white/[0.06] mx-2" />
          <div>
            <h2 className="px-3 mb-3 text-[11px] font-semibold text-zinc-500 uppercase tracking-[0.14em]">Reports & Statements</h2>
      ...
      <div className="p-3 border-t border-white/[0.06] bg-[var(--color-matte)] space-y-2.5">
        <div className="flex items-center gap-2.5 px-2.5 py-2.5 rounded-xl bg-[var(--color-matte-soft)] border border-white/[0.06]">
          ...
          <div className="flex flex-col min-w-0 flex-1">
            <span className="text-xs font-medium truncate text-white leading-none">{loading ? 'Loading...' : user?.fullName || user?.userName || 'User'}</span>
            <span className="text-[10px] text-zinc-500 truncate leading-none mt-1.5 font-mono">{user?.email || 'Active Session'}</span>
          </div>
          <Badge variant="outline" className="px-1.5 py-0 text-[10px] border-emerald-500/20 text-emerald-400 bg-emerald-500/10 rounded-md"><span className="w-1 h-1 rounded-full bg-emerald-400 animate-pulse mr-1" />Online</Badge>
        </div>
  ...
  return (
    <header className="h-16 border-b border-white/[0.06] bg-[var(--color-matte)] px-6 flex items-center justify-between shrink-0 sticky top-0 z-10 gap-4">
      ...
      <Breadcrumb className="shrink-0">
        <BreadcrumbList>
          <BreadcrumbItem><BreadcrumbLink asChild><Link href="/" className="text-zinc-500 hover:text-white text-xs">Home</Link></BreadcrumbLink></BreadcrumbItem>
          {pathSegments.map((seg, i) => {
            ...
            return (
              <div key={url} className="contents">
                ...
                <BreadcrumbItem>{isLast ? <BreadcrumbPage className="capitalize text-white text-xs">{seg.replace(/-/g, ' ')}</BreadcrumbPage> : <BreadcrumbLink asChild><Link href={url} className="capitalize text-zinc-500 hover:text-white text-xs">{seg.replace(/-/g, ' ')}</Link></BreadcrumbLink>}</BreadcrumbItem>
              </div>
            );
          })}
        </BreadcrumbList>
      </Breadcrumb>
      <div ref={containerRef} className="ml-auto hidden md:flex flex-1 justify-end max-w-xl overflow-hidden">
  ...
  return (
    <div className="flex min-h-screen w-full bg-[#f5f5f5]">
      ...
      <div className="flex flex-1 flex-col min-w-0">
        <Topbar />
        <main className="flex-1 p-6 lg:p-8"><div className="mx-auto max-w-7xl">{children}</div></main>
      </div>
    </div>
  );
}