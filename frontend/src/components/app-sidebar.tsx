...
export function AppSidebar() {
  ...
  const { state } = useSidebar(); // <-- INI KUNCINYA
  const isCollapsed = state === "collapsed";
  ...
  return (
    <Sidebar
      ...
      style={{ "--sidebar-width": "285px", "--sidebar-width-icon": "4rem" } as React.CSSProperties}
    >
      {/* HEADER FIX */}
      <SidebarHeader className={`border-b shrink-0 flex items-center gap-2 ${isCollapsed? "flex-col justify-center p-2.5 gap-3" : "flex-row justify-between p-3.5"}`}>
        {isCollapsed? (
          // PAS HIDE / COLLAPSED -> JADI ICON FAVICON
                            ...
                            {REPORT_SECTIONS.map((section) => (
                              <div key={section.title}>
                                <div className="px-2 py-1 select-none">
                                  <p className="text- font-semibold uppercase tracking-widest text-muted-foreground/60 leading-none truncate">{section.title}</p>
                                </div>
                                <div className="mt-1 flex flex-col gap-0.5">
                                  {section.items.map((sub) => (
                                    <SidebarMenuItem key={sub.url}>
                                      <SidebarMenuButton asChild isActive={pathname === sub.url} tooltip={sub.title} className="text-[13.5px] h-8 font-normal px-2">
                                        <Link href={sub.url} className="truncate">{sub.title}</Link>
                                      </SidebarMenuButton>
                                    </SidebarMenuItem>
                                  ))}
                                </div>
                ...
                <SidebarMenuButton className="w-full justify-between h-auto py-2.5 px-2">
                  <div className="flex items-center gap-2.5 text-left min-w-0">
                    ...
                    <div className="flex flex-col truncate min-w-0 group-data-[collapsible=icon]:hidden">
                      <span className="font-medium text-[13.5px] leading-tight truncate">{!isMounted || isUserLoading? "Memuat..." : user?.fullName || "Guest"}</span>
