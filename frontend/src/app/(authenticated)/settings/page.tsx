...
import {
  IconSun, IconMoon, IconDeviceDesktop, IconCheck, IconUser, IconShieldCheck, IconMail, IconId,
  IconHeartbeat, IconCircleCheck, IconAlertTriangle, IconActivity, IconDeviceLaptop, IconAlertOctagon,
  IconLogout, IconHistory, IconLoader2, IconPalette, IconLock, IconPhone, IconUpload, IconTrash, IconKey,
} from "@tabler/icons-react";
import {
  useGetApiV1AuthMeQuery, useGetApiV1SettingsGuardianDashboardQuery,
  usePostApiV1SettingsGuardianRevokeSessionBySessionIdMutation, usePostApiV1SettingsGuardianRevokeAllSessionsMutation,
  usePutApiV1SettingsProfileMutation, usePostApiV1SettingsAvatarMutation,
  usePostApiV1SettingsChangePasswordMutation, useDeleteApiV1SettingsDeleteAccountMutation,
...
interface UserProfile {
  ...
  phoneNumber?: string; bio?: string; avatarUrl?: string; roles?: string[];
}
...
export default function SettingsPage() {
  ...
  const { data: dashboardResponse, isLoading: loadingGuardian, isError, error, refetch: refetchGuardian } = useGetApiV1SettingsGuardianDashboardQuery(undefined, { skip: mainTab!== "security" });
  ...
  const handleProfileSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      showNotification("Profil berhasil diperbarui!"); refetchMe();
    } catch (err: any) { showNotification(err?.data?.message || "Gagal memperbarui profil", true); }
  };
  ...
  const handleFileChange = async (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    try {
      ...
      setAvatarPreview(res.avatarUrl); showNotification("Avatar berhasil diunggah!"); refetchMe();
    } catch (err: any) { showNotification(err?.data?.message || "Gagal mengunggah avatar", true); }
  };

  const handlePasswordSubmit = async (e: React.FormEvent) => {
    e.preventDefault();
    if (!currentPassword ||!newPassword) { showNotification("Semua field password harus diisi", true); return; }
    try {
      await changePassword({ changePasswordRequest: { currentPassword, newPassword } }).unwrap();
      showNotification("Password berhasil diubah!"); setCurrentPassword(""); setNewPassword("");
    } catch (err: any) { showNotification(err?.data?.message || "Gagal mengubah password", true); }
  };

  const handleDeleteAccount = async () => {
    if (!confirm("Apakah Anda yakin ingin menghapus akun ini? Tindakan ini tidak dapat dibatalkan!")) return;
    try { await deleteAccount().unwrap(); alert("Akun Anda telah dihapus."); window.location.href = "/login"; }
    catch (err: any) { showNotification(err?.data?.message || "Gagal menghapus akun", true); }
  };
      ...
      <div className="flex-1 overflow-y-auto pr-1 -mr-1 mt-1 space-y-4">
          ...
          <div className="space-y-4">
            <Card className="shadow-sm">
              ...
              <CardContent className="px-4 pb-4 pt-0">
                {loadingUser? (
                  <div className="flex items-center gap-2 text-xs text-muted-foreground py-4"><IconLoader2 size={16} className="animate-spin" /> Memuat data profil...</div>
                ) : user? (
                  <form onSubmit={handleProfileSubmit} className="space-y-4">
                    <div className="flex items-center gap-4">
                      <Avatar className="h-16 w-16 border">
                        <AvatarImage src={avatarPreview || undefined} />
                        <AvatarFallback className="font-bold text-sm">{(fullName || user.userName || "U").substring(0, 2).toUpperCase()}</AvatarFallback>
                      </Avatar>
                      <div>
                        ...
                        <p className="text- text-muted-foreground mt-1">JPG, PNG, WEBP max 2MB.</p>
                      </div>
                    </div>
                    <Separator />
        ...
        {mainTab === "appearance" && (
          <Card className="shadow-sm">
            <CardHeader className="py-3 px-4">
              ...
              <CardDescription className="text-xs">Tema disimpan di localStorage</CardDescription>
            </CardHeader>
            <CardContent className="px-4 pb-4 pt-0">
                ...
                {appearanceOptions.map((opt) => {
                  ...
                  return (
                    <Label key={opt.id} htmlFor={opt.id} className={cn("relative flex flex-col rounded-lg border p-3 cursor-pointer hover:bg-accent/50 transition-all", active? "border-primary bg-primary/5" : "border-muted")}>
                      ...
                      <span className="text- text-muted-foreground">{opt.desc}</span>
        ...
        {mainTab === "security" && (
          <div className="space-y-3">
            {loadingGuardian? (
              <div className="flex flex-col items-center justify-center py-16 gap-2 text-muted-foreground"><IconLoader2 className="w-6 h-6 animate-spin" /><p className="text-xs">Memuat Guardian...</p></div>
            ) : (
              <Tabs value={guardianTab} onValueChange={setGuardianTab}>
                <TabsList className="h-7 p-1 w-fit">
                  ...
                  <TabsTrigger value="sessions" className="text-xs h-5 gap-1"><IconDeviceLaptop size={12} /> Sessions <Badge variant="secondary" className="ml-1 h-4 px-1 text-">{sessions.length}</Badge></TabsTrigger>
                ...
                <TabsContent value="health" className="mt-3">
                  <div className="grid grid-cols-2 gap-3">
                    <Card className="py-3 px-3 flex items-center justify-between">
                      <div><p className="text-xs font-medium">Failed 24h</p><p className="text- text-muted-foreground">Kegagalan login</p></div>
                    ...
                    <Card className="py-3 px-3 flex items-center justify-between">
                      <div><p className="text-xs font-medium">Last Login</p><p className="text- text-muted-foreground">Waktu login terakhir</p></div>
                    ...
                    <ScrollArea className="h-64">
                      <Table>
                        <TableHeader className="sticky top-0 bg-background z-10">
                          <TableRow className="h-7"><TableHead className="text-xs h-7">Device</TableHead><TableHead className="text-xs h-7">Browser/OS</TableHead><TableHead className="text-xs h-7">IP</TableHead><TableHead className="text-xs h-7">Last</TableHead><TableHead className="text-right text-xs h-7"></TableHead></TableRow>
                        </TableHeader>
                        <TableBody>
                          {sessions.map((s: any, i: number) => (
                            <TableRow key={s.id || i} className="h-9">
                              <TableCell className="py-1.5 text-xs font-medium">{s.deviceName}{s.isCurrent && <Badge className="ml-1 h-4 text- bg-emerald-500/15 text-emerald-600">Current</Badge>}</TableCell>
                              <TableCell className="py-1.5 text-xs text-muted-foreground">{s.browser}<div className="text-">{s.operatingSystem}</div></TableCell>
                              <TableCell className="py-1.5 font-mono text-xs">{s.ipAddress}<div className="text- text-muted-foreground">{s.country}</div></TableCell>
                ...
                <TabsContent value="logs" className="mt-3">
                  <Card className="overflow-hidden">
                    ...
                    <ScrollArea className="h-64">
                      <Table>
                        <TableHeader className="sticky top-0 bg-background z-10">
                          <TableRow className="h-7"><TableHead className="text-xs h-7">Type</TableHead><TableHead className="text-xs h-7">Device</TableHead><TableHead className="text-xs h-7">IP</TableHead><TableHead className="text-xs h-7">Date</TableHead><TableHead className="text-right text-xs h-7">Status</TableHead></TableRow>
                        </TableHeader>
                        <TableBody>
                          {activities.map((a: any, i: number) => (
                            <TableRow key={a.id || i} className="h-9">
                              <TableCell className="py-1.5 text-xs">{a.activityType}</TableCell>
                              <TableCell className="py-1.5 text-xs text-muted-foreground">{a.device}<div className="text-">{a.operatingSystem}</div></TableCell>
                              ...
                              <TableCell className="py-1.5 text-right">{a.isSuccess? <Badge className="h-4 text- bg-emerald-500/15 text-emerald-600">OK</Badge> : <Badge variant="destructive" className="h-4 text-">Fail</Badge>}</TableCell>
                            </TableRow>
                          ))}
                        </TableBody>
                      </Table>
