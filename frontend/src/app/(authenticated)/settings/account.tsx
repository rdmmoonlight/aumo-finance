...
export default function AccountSettings() {
  ...
  const handleProfileSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      notify(err?.data?.message || "Gagal update profil", true);
    }
  };
  ...
  const handleFileChange = async (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    try {
      ...
      const res = await fetch(
        `${process.env.NEXT_PUBLIC_API_URL}/api/v1/settings/avatar`,
        {
          method: "POST",
  ...
  const handlePasswordSubmit = async (e: React.FormEvent) => {
    ...
    if (!currentPassword || !newPassword)
      return notify("Isi semua field password", true);
    try {
      ...
      notify(err?.data?.message || "Gagal ubah password", true);
    }
  };
  ...
  const handleDelete = async () => {
    ...
    try {
      ...
      notify(err?.data?.message || "Gagal hapus akun", true);
    }
  };
    ...
    <div className="space-y-4">
      ...
      <Card className="shadow-sm">
        ...
        <CardContent className="px-4 pb-4 pt-0">
          {loadingUser ? (
            <div className="flex gap-2 text-xs py-4">
