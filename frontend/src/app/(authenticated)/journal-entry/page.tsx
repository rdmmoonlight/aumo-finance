...
function JournalEntryContent() {
  ...
  useEffect(() => {
    const init = async () => {
      setLoading(true);
      try {
        const { data: accRes } = await apiClient.get(
        ...
        if (isEdit && entryIdParam) {
          ...
          const { data: res } = await apiClient.get(
            `/api/v1/journal-entry/${entryIdParam}`,
          );
          ...
          if (jData) {
            if (res.isLocked) {
              setLockedMessage(
                `Journal ${jData.transactionNumber} is in closed period`,
              );
            } else {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (isEdit) {
        await apiClient.put(
          `/api/v1/journal-entry/edit/${entryIdParam}`,
          payload,
        );
