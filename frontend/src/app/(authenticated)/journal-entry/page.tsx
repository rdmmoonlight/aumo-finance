...
function JournalEntryContent() {
  ...
  return (
    <div className="space-y-6 max-w-7xl">
      <div className="flex flex-wrap items-center justify-between gap-4">
        <div>
          <h2 className="text-2xl font-bold tracking-tight flex items-center gap-2">
            {isEdit ? <IconEdit className="text-primary"/> : <IconNotebook className="text-primary"/>}
            {isEdit ? 'Edit Journal Entry' : 'Create Journal Entry'}
            {isEdit && <Badge variant="secondary" className="font-mono text-xs">{transactionNumber}</Badge>}
          </h2>
          <p className="text-sm text-muted-foreground mt-1">Record double-entry transactions</p>
        </div>
        <Button variant="outline" size="sm" asChild>
          <Link href="/reports/general-journal" className="gap-1.5"><IconArrowLeft size={14}/> Back to Journal</Link>
        </Button>
      </div>
      ...
      {lockedMessage ? (
        <Alert><IconLock size={16}/><AlertDescription>{lockedMessage} <Link href="/reports/general-journal" className="underline">Back</Link></AlertDescription></Alert>
      ) : (
        <form onSubmit={handleSubmit} className="space-y-6">
          <Card>
            <CardHeader className="pb-3"><CardTitle className="text-sm">Transaction Info</CardTitle></CardHeader>
            <CardContent className="grid grid-cols-1 md:grid-cols-3 gap-4">
              ...
              <div className="space-y-1.5">
                ...
                <Input type="date" className="h-9" value={entryDate} onChange={e => setEntryDate(e.target.value)} required />
              </div>
            </CardContent>
          </Card>
              ...
              <Table>
                ...
                <TableFooter>
                  <TableRow>
                    ...
                    <TableCell className="text-right font-mono text-emerald-500">Rp {formatIDR(totalDebit)}</TableCell>
                    <TableCell className="text-right font-mono text-red-500">Rp {formatIDR(totalCredit)}</TableCell>
                    <TableCell/>
                  </TableRow>
                  <TableRow>
                    <TableCell colSpan={3} className="text-right">Status:</TableCell>
                    <TableCell colSpan={2} className="text-center">
                      {isBalanced ? (
                        <Badge className="bg-emerald-500/15 text-emerald-600 border-emerald-500/20 gap-1"><IconCircleCheck size={12}/> Balanced</Badge>
                      ) : (
                        <Badge variant="destructive" className="gap-1 bg-red-500/15 text-red-500 border-red-500/20"><IconAlertTriangle size={12}/> Unbalanced Rp {formatIDR(Math.abs(totalDebit - totalCredit))}</Badge>
                      )}
                    </TableCell>
                    <TableCell/>
                  </TableRow>
...
export default function JournalEntryPage() {
  return (
    <Suspense fallback={<div className="py-16 text-center text-sm text-muted-foreground flex items-center justify-center gap-2"><IconLoader2 className="animate-spin" size={16}/> Loading...</div>}>
      <JournalEntryContent/>
    </Suspense>
  );
}
