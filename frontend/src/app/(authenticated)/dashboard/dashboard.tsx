...
import { DashboardViewModel, getDashboardData } from "./actions";
...
export default function DashboardClient({
  ...
  const handlePeriodSwitch = (type: "monthly" | "annual") => {
    ...
    startTransition(async () => {
      ...
      const { data: newData, error } = await getDashboardData(type);
      if (error) {
  ...
  const healthScore = useMemo(() => {
    ...
    const margin =
      data.totalRevenue > 0 ? (data.netIncome / data.totalRevenue) * 100 : 0;
  ...
  const doughnutCashData = useMemo(
    () => ({
      ...
      datasets: [
        {
          data: data ? [data.totalCashOnHand, data.totalBankBalance] : [0, 0],
  ...
  const expenseChartData = useMemo(
    () => ({
      labels: data?.expenseAccountsList?.length
        ? data.expenseAccountsList.map((i) => i.accountName)
        : ["No Expenses"],
      datasets: [
        {
          data: data?.expenseAccountsList?.length
            ? data.expenseAccountsList.map((i) => i.balance)
            : [1],
          backgroundColor: [
            "#ef4444",
            "#f59e0b",
  ...
  const barTrendData = useMemo(
    () => ({
      ...
      datasets: [
        {
          label: "Revenue",
          data: data?.chartTrend.map((t) => t.revenue) || [],
          backgroundColor: "#10b981",
          borderRadius: 4,
        },
        {
          label: "Expenses",
          data: data?.chartTrend.map((t) => t.expense) || [],
  ...
  const lineNetData = useMemo(
    () => ({
      ...
      datasets: [
        {
          label: "Net Income",
          data: data?.chartTrend.map((t) => t.net) || [],
          borderColor: "#6366f1",
      ...
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
        <div>
          ...
          <p className="text-sm text-muted-foreground">
            ...
            • In IDR{" "}
            {isPending && (
              <span className="animate-pulse ml-2">• Fetching...</span>
            )}
          </p>
        </div>
            ...
            <div className="text-xs text-muted-foreground flex gap-4 mt-1">
              <span>
                ...
                <b className="text-foreground">
                  {formatNumber(data.totalCashOnHand)}
                </b>
              </span>
              <span>
                Bank:{" "}
                <b className="text-foreground">
                  {formatNumber(data.totalBankBalance)}
                </b>
              </span>
            </div>
          </CardContent>
        ...
        <Card className="bg-primary text-primary-foreground">
          ...
          <CardContent>
            ...
            <p className="text-xs text-primary-foreground/70">
              {data.netIncome >= 0 ? "Profit" : "Loss"} for Period
            </p>
          </CardContent>
        </Card>
        <Card>
