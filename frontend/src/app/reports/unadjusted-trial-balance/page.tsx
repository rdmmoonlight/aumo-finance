...
import { IconList, IconCalendar, IconEyeOff, IconAlertTriangle, IconLoader2, IconCircleCheck } from '@tabler/icons-react';
    ...
    <Card className="overflow-hidden"><CardContent className="p-0">
      ...
      <TableBody>
        {rows.length? rows.map(r => (
          <TableRow key={r.accountId}><TableCell className="text-center pl-6"><Badge variant="outline" className="font-mono text-amber-500">{r.referenceNumber || '-'}</Badge></TableCell>
          <TableCell className="text-xs font-medium">{r.accountName}</TableCell>
          <TableCell><Badge variant="secondary">{r.type}</Badge></TableCell>
          <TableCell className="text-right font-mono text-xs text-emerald-500">{(r.debit || 0) > 0? formatNumber(r.debit!) : '-'}</TableCell>
          <TableCell className="text-right pr-6 font-mono text-xs text-red-500">{(r.credit || 0) > 0? formatNumber(r.credit!) : '-'}</TableCell></TableRow>
        )) : <TableRow><TableCell colSpan={5} className="text-center py-8 text-muted-foreground text-xs">No accounts found.</TableCell></TableRow>}
      </TableBody>
...
export default function UnadjustedTrialBalancePage() {
  const { noPeriod, loading, error, rows, totalDebit, totalCredit, isBalanced } = useTrialBalance('/api/v1/reports/trial-balance/unadjusted');

  if (loading) return <div className="py-16 text-center text-muted-foreground flex items-center justify-center gap-2"><IconLoader2 className="animate-spin" size={16}/> Loading unadjusted trial balance...</div>;

  return (
    <div className="space-y-6">
      <div className="flex flex-wrap items-center justify-between gap-3">
        <div><h1 className="text-xl font-bold flex items-center gap-2"><IconList className="text-sky-500" size={22}/> Unadjusted Trial Balance</h1><p className="text-sm text-muted-foreground mt-1">Before adjustments • IDR</p></div>
      </div>
      ...
      {noPeriod? (
        ...
        <>
          ...
          <Alert className={isBalanced? 'bg-emerald-500/10 border-emerald-500/20' : 'bg-red-500/10 border-red-500/20'}><IconCircleCheck size={16}/><AlertDescription className="text-xs">{isBalanced? 'Balanced: Debit = Credit' : 'Unbalanced - check journal entries'}</AlertDescription></Alert>
        </>
      )}
    </div>
  );
