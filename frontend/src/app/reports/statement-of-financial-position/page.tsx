...
export default function StatementOfFinancialPositionPage() {
  ...
  return (
    <div className="space-y-6">
      ...
      {noPeriod? (
        <Card className="py-16 text-center border-dashed"><CardContent className="space-y-3"><IconEyeOff size={36} className="mx-auto text-muted-foreground"/><h3 className="font-semibold">No Period Selected</h3><Button asChild size="sm"><Link href="/periods" className="gap-1.5"><IconCalendar size={14}/> Go to Periods</Link></Button></CardContent></Card>
      ) : (
        <>
          <div className="flex flex-wrap items-center justify-between gap-3"><div><h1 className="text-xl font-bold flex items-center gap-2"><IconBuildingBank className="text-sky-500" size={22}/> Statement of Financial Position</h1><p className="text-sm text-muted-foreground mt-1">As of {formatDateDisplay(vm.asOfDate)||'current period'} • IAS 1 • IDR</p></div><Button asChild variant="outline" size="sm" className="gap-1.5"><Link href="/reports/closing-journal"><IconArrowRight size={14}/> Closing Journal</Link></Button></div>
