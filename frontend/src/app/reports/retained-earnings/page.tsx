...
export default function RetainedEarningsPage() {
  ...
  return (
    <div className="space-y-6">
      ...
      {noPeriod? (
        <Card className="py-16 text-center border-dashed"><CardContent className="space-y-3"><IconEyeOff size={36} className="mx-auto text-muted-foreground"/><h3 className="font-semibold">No Period Selected</h3><p className="text-sm text-muted-foreground">This report follows whichever period you're viewing.</p><Button asChild size="sm"><Link href="/periods" className="gap-1.5"><IconCalendar size={14}/> Go to Periods</Link></Button></CardContent></Card>
      ) : (
        <>
          <div className="flex flex-wrap items-center justify-between gap-3"><div><h1 className="text-xl font-bold flex items-center gap-2"><IconPigMoney className="text-emerald-500" size={22}/> Retained Earnings Statement</h1><p className="text-sm text-muted-foreground mt-1">Bridges Income Statement to Equity on Balance Sheet • IDR</p></div><Button asChild variant="outline" size="sm" className="gap-1.5"><Link href="/reports/statement-of-financial-position"><IconArrowRight size={14}/> Balance Sheet</Link></Button></div>
